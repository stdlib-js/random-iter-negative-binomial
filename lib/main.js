...
function iterator( r, p, options ) {
	...
	if ( !isPositiveNumber( r ) ) {
		throw new TypeError( format( '0qB4b', r ) );
	}
	if ( !isProbability( p ) ) {
		throw new TypeError( format( '0qB73', p ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0qB2V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0qB2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
