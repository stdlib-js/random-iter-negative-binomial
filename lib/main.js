...
function iterator( r, p, options ) {
	...
	if ( !isPositiveNumber( r ) ) {
		throw new TypeError( format( '0Pz4o', r ) );
	}
	if ( !isProbability( p ) ) {
		throw new TypeError( format( '0Pz7L', p ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pz2h', options ) );
		}
		opts = copy( options, 1 );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pz35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
