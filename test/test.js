...
tape( 'the function throws an error if `r`, the number of successes until experiment is stopped, is not a positive number', function test( t ) {
	var values;
	var i;
...
tape( 'the function throws an error if success probability `p` is not a probability', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns an iterator protocol-compliant object (seed)', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		r1 = it1.next();
		r2 = it2.next();
		t.strictEqual( r1.value, r2.value, 'both return same number' );
	}
	t.end();
});
...
tape( 'attached to the returned iterator is the underlying PRNG', function test( t ) {
	...
	t.strictEqual( typeof it.PRNG, 'function', 'has property' );

	it = iterator( 10, 0.3, {
		'prng': minstd.normalized
	});
	t.strictEqual( it.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned iterator is the generator seed', function test( t ) {
	...
	t.strictEqual( isUint32Array( it.seed ), true, 'has property' );
	t.strictEqual( it.seed[ 0 ], 12345, 'equal to provided seed' );

	it = iterator( 10, 0.3, {
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( it.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned iterator is the generator seed length', function test( t ) {
	var it = iterator( 10, 0.3 );
	t.strictEqual( typeof it.seedLength, 'number', 'has property' );

	it = iterator( 10, 0.3, {
		'prng': minstd.normalized
	});
	t.strictEqual( it.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned iterator is the generator state', function test( t ) {
	var it = iterator( 10, 0.3 );
	t.strictEqual( isUint32Array( it.state ), true, 'has property' );

	it = iterator( 10, 0.3, {
		'prng': minstd.normalized
	});
	t.strictEqual( it.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned iterator is the generator state length', function test( t ) {
	var it = iterator( 10, 0.3 );
	t.strictEqual( typeof it.stateLength, 'number', 'has property' );

	it = iterator( 10, 0.3, {
		'prng': minstd.normalized
	});
	t.strictEqual( it.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned iterator is the generator state size', function test( t ) {
	var it = iterator( 10, 0.3 );
	t.strictEqual( typeof it.byteLength, 'number', 'has property' );

	it = iterator( 10, 0.3, {
		'prng': minstd.normalized
	});
	t.strictEqual( it.byteLength, null, 'equal to `null`' );
	t.end();
});

tape( 'the function returns an iterator for generating pseudorandom numbers from a negative binomial distribution', function test( t ) {
	var rand;
	var it;
	var i;
	...
	for ( i = 0; i < 1e3; i++ ) {
		t.strictEqual( rand(), it.next().value, 'returns expected value' );
	}
	t.end();
});
...
tape( 'the function supports limiting the number of iterations', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		...
		j = i + 1;
		if ( j <= niter ) {
			t.strictEqual( typeof r.value, 'number', 'returns expected `value` value. iteration: '+j );
			t.strictEqual( r.done, false, 'returns expected `done` value. iteration: '+j );
		} else {
			t.strictEqual( r.value, void 0, 'returns expected `value` value. iteration: '+j );
			t.strictEqual( r.done, true, 'returns expected `done` value. iteration: '+j );
		}
	}
	t.end();
});
...
tape( 'the function supports specifying the underlying PRNG', function test( t ) {
	...
	for ( i = 0; i < 1e2; i++ ) {
		r = it.next().value;
		t.strictEqual( typeof r, 'number', 'returns expected value' );
	}
	t.end();
});
...
tape( 'the function supports providing a seeded underlying PRNG', function test( t ) {
	...
	for ( i = 0; i < 1e2; i++ ) {
		r1 = it1.next().value;
		r2 = it2.next().value;
		t.strictEqual( r1, r2, 'both return same number' );
	}
	t.end();
});
...
tape( 'the function supports specifying the underlying generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( it.next().value, arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
...
tape( 'the function supports specifying a shared underlying generator state', function test( t ) {
	...
	for ( i = 0; i < 25; i++ ) {
		v1 = it1.next().value;
		v2 = it2.next().value;
		t.strictEqual( v1, arr[ j ], 'returns expected value. i: '+j+'.' );
		t.strictEqual( v2, arr[ j+1 ], 'returns expected value. i: '+(j+1)+'.' );
		j += 2; // stride
	}
